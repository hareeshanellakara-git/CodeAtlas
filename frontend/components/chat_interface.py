...
def render_chat_interface():
    ...
    if not st.session_state.repo_indexed:
        st.info("Analyze a repository above to start chatting.")
        return
...
def _render_input():
    question = st.chat_input("Ask CodeAtlas about this repository...")
    if not question:
        return
    ...
    with st.chat_message("assistant"):
        with st.spinner("🔍 Searching the codebase and asking the LLM..."):
            try:
                ...
                add_chat_message("assistant", result.answer, result.sources)
